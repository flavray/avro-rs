...
fn make_big_json_record() -> serde_json::Value {
...
fn make_json_records(record: &serde_json::Value, count: usize) -> Vec<serde_json::Value> {
...
fn write_json(records: &[serde_json::Value]) -> Vec<u8> {
...
fn bench_read_json(b: &mut test::Bencher, make_record: &Fn() -> (serde_json::Value), n_records: usize) {
